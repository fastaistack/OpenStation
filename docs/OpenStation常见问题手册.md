...
OpenStation支持部署多种**DeepSeek**等业内常见模型，并提供一键部署功能。以下是部分模型列表、下载地址及其资源需求：
...
### 4.5 平台部署模型服务开启工具调用或已部署模型调用工具错误
需要部署模型支持工具调用，并在部署时开启相关参数。当前平台在部署DeepSeek及千问系列模型时，会自动开启部署参数。其他系列模型在部署时需要在`高级配置`的扩展参数中设置，请查询模型官方文档确认是否支持工具调用及开启时需要使用的参数。
比如使用Kimi-K2模型，查询相关文档，需要在部署时添加扩展参数：
```bash
# vLLM引擎
