...
* :fire: \[2025-12-25] **OpenStation V0.6.8发布，优化功能，支持添加模型卡片！**
* :fire: \[2025-12-01] **OpenStation V0.6.7发布，新增多种模型支持，加强资源管理能力，支持指定卡部署模型！**
...
#其中，--version 0.6.8表示本次安装OpenStation平台的版本
bash openstation-install-online.sh  --version 0.6.8
```
您也可以通过链接直接下载[在线安装包openstation-pkg-online-v0.6.8.tar.gz](https://fastaistack.oss-cn-beijing.aliyuncs.com/openstation/openstation-pkg-online-v0.6.8.tar.gz)，上传到服务器上执行以下安装命令：
```shell
...
安装过程中，您需输入 **本机节点 IP 地址** 及 **选择部署模式：cpu 或者 gpu** 等信息，安装程序将根据配置自动完成 **OpenStation** 的部署。
...
| **包类型**       | **包下载地址**                                                                                       | **内容**        | **大小** |
| ------------- | ----------------------------------------------------------------------------------------------- | ------------- | ------ |
| **离线安装包**     | [点击下载](https://fastaistack.oss-cn-beijing.aliyuncs.com/openstation/openstation-pkg-v0.6.8.tar.gz) | 平台功能组件+推理引擎镜像 | 10.1GB |
| **安装包SHA256** | [点击下载](https://fastaistack.oss-cn-beijing.aliyuncs.com/openstation/SHA256SUM-0.6.8)                     | 安装包SHA256校验文件 | 96B    |
...
```shell
#校验包的完整性
sha256sum -c SHA256SUM-0.6.8
#解压安装包，执行如下安装过程
tar -xvzf openstation-pkg-v0.6.8.tar.gz && cd openstation-pkg-v0.6.8/deploy && bash install.sh
```
